...
namespace AI_School
{
    public partial class Form_StudentsClasses : Form
    {
        ...
        {
            Form_StudentsClassesDlg frm = new Form_StudentsClassesDlg();
                ...
                if (ConnectToSQLiteDB() == 1)
                {
                    using (SQLiteCommand fmd = con.CreateCommand())
                    {
                        try
                        {
                            ...
                            fmd.CommandText = @"DELETE FROM studentsClasses WHERE id = @l";
        ...
        private void button_refresh_Click(object sender, EventArgs e)
        {
            if (ConnectToSQLiteDB() == 1)
            {
                ...
                try
                {
                    string sql = @"SELECT id as '#',className as 'Найменування класу',studentsCount as 'Кількість',classTeacher as 'Класний керівник',comment as 'Примітка' FROM studentsClasses"; // можливо треба поставити LIMIT ?
                    SQLiteDataAdapter adapter = new SQLiteDataAdapter(sql,con);
                    DataSet ds = new System.Data.DataSet();
                    adapter.Fill(ds, "studentsClasses");
