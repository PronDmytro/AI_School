...
namespace AI_School
{
    public partial class Form_Supplylog : Form
    {
        ...
        private void button_refresh_Click(object sender, EventArgs e)
        {
            if (ConnectToSQLiteDB() == 1)
            {
                ...
                try
                {
                    string sql = @"SELECT schedule.id as '#', teachers.name as 'Вчитель', studentsClasses.className as 'Клас',schedule.lesson_dt as 'Дата уроку',schedule.lessonNumber as 'Номер уроку',schedule.subject as 'Назва уроку' FROM schedule LEFT JOIN teachers ON schedule.teacherId = teachers.id LEFT JOIN studentsClasses ON schedule.studentsClassId = studentsClasses.id"; // можливо треба поставити LIMIT ?
                ...
                if (ConnectToSQLiteDB() == 1)
                {
                    using (SQLiteCommand fmd = con.CreateCommand())
                    {
                        try
                        {
                            ...
                            fmd.CommandText = @"DELETE FROM schedule WHERE id = @l";
        ...
        private void button_edit_Click(object sender, EventArgs e)
        {
            ...
            Form_SupplylogDlg frm = new Form_SupplylogDlg();
