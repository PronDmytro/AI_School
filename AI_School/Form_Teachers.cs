    ...
    public partial class Form_Teachers : Form
    {
        ...
        private void button_refresh_Click(object sender, EventArgs e)
        {
            if (ConnectToSQLiteDB() == 1)
            {
                ...
                try
                {
                    string sql = @"SELECT id as '#',name as 'ПІБ вчителя',experience as 'Стаж' FROM teachers"; // можливо треба поставити LIMIT ?
                ...
                if (ConnectToSQLiteDB() == 1)
                {
                    using (SQLiteCommand fmd = con.CreateCommand())
                    {
                        try
                        {
                            ...
                            fmd.CommandText = @"DELETE FROM teachers WHERE id = @l";
        ...
        private void button_edit_Click_1(object sender, EventArgs e)
        {
            ...
            Form_TeachersDlg frm = new Form_TeachersDlg();
